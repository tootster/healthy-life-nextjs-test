...
export default function Home({data}) {
  ...
  return (
    <div className=" bg-cover bg-center" style={{ backgroundImage: `url(https://images.unsplash.com/photo-1509099836639-18ba1795216d?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1530&q=80)`}}>
      ...
      <div class="h-30 w-15 mt-16 mb-5 ml-7">
        <h2 class="text-5xl text-left font-light" >{Object.keys(data).length} GOALS TO </h2>
        <h2  class="text-5xl text-left font-light">TRANSFORM OUR</h2>
        <h2 class="text-5xl text-left font-light">WORLD</h2>
      </div>
      <div class="grid grid-cols-2 gap-4">
        {data.map((goal) => (
            <Link href={{pathname: `/goal/${goal.code}`}} class={`m-2 ${getRandomColor()} `} key={goal.code}>
              ...
              <div class="flex ">
                <h1 class="text-4xl  font-bold m-4">{goal.code}</h1> 
                <h2 class="text-lg">{goal.title}</h2>
              </div>
      ...
      <Link class="mt-10 h-20 bg-blue-400 flex items-center justify-center text-2xl font-bold mb-4 " href={`aboutus`}>
        ABOUT US 
      </Link>
    </div>
  );
